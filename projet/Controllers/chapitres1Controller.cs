...
{
    public class chapitres1Controller : Controller
    {
        private readonly PrjContext _context; private readonly IHostingEnvironment _hostingEnvironment;


        public chapitres1Controller(PrjContext context, IHostingEnvironment hostingEnvironment)
        {
        ...
        public async Task<IActionResult> Create(ChapitreViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                ViewBag.nom = HttpContext.Session.GetString("nom");
                ViewBag.prenom = HttpContext.Session.GetString("prenom");
                return RedirectToAction("Index1", "Modules",ModulesController.mode);
            }
            ViewBag.id_mod = new SelectList(_context.Modules, "id_mod", "nom_mod");
            ViewBag.nom = HttpContext.Session.GetString("nom");
            ViewBag.prenom = HttpContext.Session.GetString("prenom");
